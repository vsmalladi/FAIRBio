...
class GA4GHServiceRegistry:
    ...
    def __init__(self, registry_url: Optional[str] = None):
        ...
        Args:
            registry_url: Base URL of the GA4GH service registry (default: official GA4GH registry)
        """
    ...
    def get_services(self) -> List[Dict]:
        ...
        Returns:
            List of service configurations with metadata
        """
        try:
    ...
    def get_service_types(self) -> List[Dict]:
        ...
        Returns:
            List of service type configurations
        """
        try:
            url = f"{self.registry_url}services/types"
            response = self.session.get(url, timeout=10)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
    ...
    def get_service_info(self) -> Optional[Dict]:
        ...
        Returns:
            Service information about the registry
        """
        try:
            url = f"{self.registry_url}service-info"
            response = self.session.get(url, timeout=10)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
            print(f"Error fetching registry info: {e}")
            return None
    ...
    def get_services_by_type(self, service_type: str) -> List[Dict]:
        ...
        Args:
            service_type: Type of service to filter for (e.g., 'TRS', 'WES', 'TES', 'DRS')
        ...
        for service in all_services:
            ...
            if isinstance(service_type_obj, dict):
                ...
                if service_type in artifact or service_type.lower() in str(service_type_obj):
